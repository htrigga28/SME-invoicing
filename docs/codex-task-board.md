...
| T021 Invoice Delivery, View Tracking and Unified Activity | Done | `feat/t021-invoice-delivery-activity` | [#27](https://github.com/htrigga28/SME-invoicing/pull/27) | Add Resend invoice delivery with To/CC, signed delivery webhooks, repeated public view tracking, manual resend/retry safety, and one unified invoice lifecycle timeline. | Invoice detail explains issuance, delivery, views, payments, reconciliation, refunds, and receipts while preserving financial truth and tenant isolation. |
| T022 Recurring Billing, Scheduled Sends and Reminder Automation | In Review | `feat/t022-recurring-reminder-automation` | [#29](https://github.com/htrigga28/SME-invoicing/pull/29) | Add recurring invoice schedules, date-based scheduled sends, organisation reminder sequences, customer/invoice reminder suppression, and a Postgres-backed automation job ledger driven by one protected daily Vercel Cron invocation. | Recurring invoices and reminders execute idempotently without duplicate invoices/emails, reuse T021 Resend communications, respect payment truth, and expose clear pause/resume/failure states in the Lumina workspace. |
...
- MKT-01 shipped in PR #23. T021 shipped in PR #27. T022 Recurring Billing / Scheduled Sends / Reminder Automation is now the next product task.
...
- T022 execution authority: `docs/lumina-v2/15_T022_RECURRING_REMINDER_AUTOMATION_CODEX_PROMPT.md`. Reuse T021 Resend/communications, keep automation date-based in Africa/Lagos, and use a Postgres-backed job ledger plus one protected daily Vercel Cron trigger.
