...
describe("RolesGuard", () => {
  ...
  it("fails closed on mutations when the workspace header is omitted", async () => {
    const reflector = {
      getAllAndOverride: jest.fn((key: string) =>
        key === ROLES_KEY ? ["accountant"] : undefined
      )
    ...
    const tenantContextService = {
      resolveForUser: jest.fn(
        async (_userId: string, _orgId: unknown, options?: { requireExplicit?: boolean }) => {
          if (options?.requireExplicit) {
            throw new Error("Select a workspace before changing business data.");
          }
    ...
    ).rejects.toThrow("Select a workspace before changing business data.");
    expect(tenantContextService.resolveForUser).toHaveBeenCalledWith("user-1", undefined, {
      requireExplicit: true
    });
  });
